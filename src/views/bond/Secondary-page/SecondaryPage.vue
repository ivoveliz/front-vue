  ...
  <section class="grid-view wishlist-items">
    ...
    >
      <div class="item-img text-center">
        <b-link :to="{ name: 'bond-Entity-page', params: { slug: product.EntityAsociated} }">
          <b-img
            :alt="`${product.NameSecondaryGroup}-${product.IdSecondaryGroup}`"
            fluid
            class="card-img-top"
            :src="require('@/assets/images/pages/eCommerce/4.png')"
      ...
      <b-card-body>
        ...
        <b-card-text class="item-description">
          {{ product.IdSecondaryGroup }}
        </b-card-text>
    
         <b-card-text class="item-description">
          Cantidad entidades : {{ product.AmountEntity }}
        </b-card-text>
  ...
  setup() {
    ...
    const fetchWishlistProducts = () => {
     if(productId){
      ...
      store.dispatch('app-bond/fetchSecondaryGroup', { productId})
        .then(response => {
       // console.log(response)
        })
     }else{
      store.dispatch('app-bond/fetchSecondarySaved', { productId})
        .then(response => {
